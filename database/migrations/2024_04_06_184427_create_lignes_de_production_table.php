...
/**
 * Class ***`CreateLignesDeProductionTable`***
 *
 * A migration class for creating the "lignes_de_production" table with UUID primary key and timestamps.
 *
 * @package ***`\Database\Migrations\CreateLignesDeProductionTable`***
 */
class CreateLignesDeProductionTable extends Migration
{
    ...
    public function up(): void
    {
        ...
        try {
            ...
            Schema::create('lignes_de_production', function (Blueprint $table) {
                // Define a UUID primary key for the 'lignes_de_production' table
                $this->uuidPrimaryKey($table);

                // Define a string column 'name' to store the description or label of the ecriture comptable (accounting entry).
                $table->string('name')
                    ->comment('Nom du projet de production');
            ...
            throw new \Core\Utils\Exceptions\DatabaseMigrationException(
                message: 'Failed to migrate "lignes_de_production" table: ' . $exception->getMessage(),
                previous: $exception
            );
        }
    }
    ...
    public function down(): void
    {
        ...
        try {
            // Drop the "lignes_de_production" table if it exists
            Schema::dropIfExists('lignes_de_production');
            ...
            // Handle the exception (e.g., logging, notification, etc.)
            throw new \Core\Utils\Exceptions\DatabaseMigrationException(
                message: 'Failed to drop "lignes_de_production" table: ' . $exception->getMessage(),
                previous: $exception
            );
        }
    }
